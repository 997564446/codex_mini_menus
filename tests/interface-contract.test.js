...
test('厨师和食客页面包含分类双列与当前分类全选入口', () => {
  ...
  assert.match(mealEdit, /每餐默认勾选，不可取消/)
  assert.match(mealEdit, /disabled="\{\{item\.isRequiredDish\}\}"/)
...
test('星期菜单支持按餐别同步到厨师选择的多个星期', () => {
  const menuSource = fs.readFileSync(path.join(root, 'cloudfunctions/menu/index.js'), 'utf8')
  const mealEdit = fs.readFileSync(path.join(root, 'miniprogram/pages/meal-edit/index.wxml'), 'utf8')
  const mealEditLogic = fs.readFileSync(path.join(root, 'miniprogram/pages/meal-edit/index.js'), 'utf8')
  assert.match(menuSource, /case 'syncMeals'/)
  assert.match(menuSource, /targetVersions/)
  assert.match(menuSource, /startTransaction\(\)/)
  assert.match(mealEdit, /同步到哪些天/)
  assert.match(mealEdit, /将覆盖所选日期原有的\{\{mealTypeLabel\}\}菜单/)
  assert.match(mealEditLogic, /请先保存当前菜单/)
  assert.match(mealEditLogic, /api\.call\('menu', 'syncMeals'/)
})
